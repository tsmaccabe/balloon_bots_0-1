...
fn simulate(behavior_policy: Policy, mut scene_window: &mut Window, cam: &mut kiss3d::camera::FirstPerson, mut rigid_body_set: RigidBodySet, mut collider_set: ColliderSet, mut joint_set: ImpulseJointSet, mut body: &mut Body, body_visual: &mut RigidBodyVisual, body_radius: f32, num_frames: u32, gravity: rna::Vector2<f32>, sensor_rangers: Vec<Ranger>) -> f32 {
    ...
    for i in 0..num_frames {
        ...
        unwrap_joint_and_set_motor_position(&mut joint_set, &mut body.legs.0.knee, -PI/6. - PI/8. * ((i as f32)*PI/30. - PI).sin());
        unwrap_joint_and_set_motor_position(&mut joint_set, &mut body.legs.1.knee, -PI/6. - PI/8. * ((i as f32)*PI/30. - PI/3.).sin());
        unwrap_joint_and_set_motor_position(&mut joint_set, &mut body.hips.0, PI/4. + PI/8. * ((i as f32)*PI/30.).sin());
        unwrap_joint_and_set_motor_position(&mut joint_set, &mut body.hips.1, -PI/4. - PI/8. * ((i as f32)*PI/30. + 2.*PI/3.).sin());
